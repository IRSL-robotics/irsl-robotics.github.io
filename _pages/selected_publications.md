...
<script>
  (() => {
    const initializePublicationEnhancements = () => {
      ...
      Object.entries(githubLinks).forEach(([bibKey, githubUrl]) => {
        const links = document.getElementById(bibKey)?.querySelector(".links");
        if (!links || links.querySelector(".github-link")) return;

        const githubLink = document.createElement("a");
        githubLink.href = githubUrl;
        githubLink.className = "github-link btn btn-sm z-depth-0";
